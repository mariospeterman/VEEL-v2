...
5. Fetch protected `main`; inspect its latest CI and query open pull requests carrying
   `wevid-active-slice`.
6. Exactly one labelled pull request means resume it and verify that its branch matches the planned
   slice contract. More than one means stop writes and repair the mutex.
7. With no labelled pull request, choose `nextPlannedSlice` from
   `docs/v2-new-build/production-status.json` and confirm INDEX/build-plan/status agreement.
...
2. Keep transient branch, pull-request, head-SHA, CI, and review truth in GitHub only. Protected-main
   docs keep merged baseline/evidence/blockers and the next planned slice.
...
6. Fetch the squash merge, verify main-branch CI at the merge SHA, delete the merged branch, and
   advance the stable production-status fragment to the merged SHA/migration/evidence and next slice.
