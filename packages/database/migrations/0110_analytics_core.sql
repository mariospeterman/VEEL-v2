...
create table analytics_creator_daily (
  ...
  feed_impression_count bigint not null default 0 check (feed_impression_count >= 0),
  profile_open_count bigint not null default 0 check (profile_open_count >= 0),
  follow_after_view_count bigint not null default 0 check (follow_after_view_count >= 0),
...
create table analytics_creator_product_daily (
  ...
  offer_impression_count bigint not null default 0 check (offer_impression_count >= 0),
  membership_start_count bigint not null default 0 check (membership_start_count >= 0),
  membership_cancel_count bigint not null default 0 check (membership_cancel_count >= 0),
  updated_at timestamptz not null default now(),
...
create table analytics_viewer_daily (
  bucket_date date not null,
  ...
  app_session_count bigint not null default 0 check (app_session_count >= 0),
  return_session_count bigint not null default 0 check (return_session_count >= 0),
  updated_at timestamptz not null default now(),
  primary key (bucket_date, user_id)
);

create table analytics_platform_commerce_daily (
  bucket_date date not null,
  currency text not null check (currency in ('SOL', 'USDC')),
  confirmed_purchase_count bigint not null default 0 check (confirmed_purchase_count >= 0),
  confirmed_gross_minor bigint not null default 0 check (confirmed_gross_minor >= 0),
  posted_platform_fee_minor bigint not null default 0 check (posted_platform_fee_minor >= 0),
  posted_referral_commission_minor bigint not null default 0 check (posted_referral_commission_minor >= 0),
  confirmed_management_allocation_minor bigint not null default 0 check (confirmed_management_allocation_minor >= 0),
  updated_at timestamptz not null default now(),
  primary key (bucket_date, currency)
);

create table analytics_platform_operations_daily (
  bucket_date date primary key,
  moderation_job_count bigint not null default 0 check (moderation_job_count >= 0),
  moderation_decision_seconds bigint not null default 0 check (moderation_decision_seconds >= 0),
  provider_failure_count bigint not null default 0 check (provider_failure_count >= 0),
  worker_retry_count bigint not null default 0 check (worker_retry_count >= 0),
  worker_dead_letter_count bigint not null default 0 check (worker_dead_letter_count >= 0),
  updated_at timestamptz not null default now()
);

create table analytics_retention_daily (
  activity_date date not null,
  cohort_date date not null,
  active_user_count bigint not null default 0 check (active_user_count >= 0),
  updated_at timestamptz not null default now(),
  primary key (activity_date, cohort_date),
  check (activity_date >= cohort_date)
);

create table analytics_onboarding_daily (
  bucket_date date not null,
  ...
  distinct_journey_count bigint not null default 0 check (distinct_journey_count >= 0),
  updated_at timestamptz not null default now(),
  primary key (bucket_date, event_key)
);

create table analytics_profile_open_receipts (
  ...
  check (actor_user_id <> profile_user_id),
  check (expires_at > created_at)
);

create table analytics_offer_impression_receipts (
  user_id uuid not null references users(id) on delete cascade,
  creator_user_id uuid not null references users(id) on delete cascade,
  product_type text not null check (product_type in (
    'support', 'content_unlock', 'paid_message', 'live_pass', 'event_access_pass', 'membership'
  )),
  currency text not null check (currency in ('SOL', 'USDC')),
...
create table analytics_onboarding_journey_events (
  ...
  primary key (journey_id, idempotency_key),
...
create table analytics_privacy_suppression_daily (
  ...
  scope_type text not null check (scope_type in ('viewer', 'creator', 'organization', 'platform')),
...
create index analytics_viewer_user_date_idx
  on analytics_viewer_daily (user_id, bucket_date desc);
create index analytics_platform_commerce_date_idx
  on analytics_platform_commerce_daily (bucket_date desc, currency);
create index analytics_retention_cohort_date_idx
  on analytics_retention_daily (cohort_date, activity_date desc);
create index analytics_profile_open_profile_date_idx
  on analytics_profile_open_receipts (profile_user_id, created_at desc);
create index analytics_offer_impression_creator_date_idx
  on analytics_offer_impression_receipts (creator_user_id, created_at desc, product_type);
create index analytics_onboarding_event_date_idx
  on analytics_onboarding_journey_events (event_key, occurred_at desc);
...
alter table analytics_viewer_daily enable row level security;
alter table analytics_platform_commerce_daily enable row level security;
alter table analytics_platform_operations_daily enable row level security;
alter table analytics_retention_daily enable row level security;
alter table analytics_onboarding_daily enable row level security;
alter table analytics_profile_open_receipts enable row level security;
alter table analytics_offer_impression_receipts enable row level security;
alter table analytics_onboarding_journey_events enable row level security;
...
revoke all on table analytics_viewer_daily from public, anon, authenticated;
revoke all on table analytics_platform_commerce_daily from public, anon, authenticated;
revoke all on table analytics_platform_operations_daily from public, anon, authenticated;
revoke all on table analytics_retention_daily from public, anon, authenticated;
revoke all on table analytics_onboarding_daily from public, anon, authenticated;
revoke all on table analytics_profile_open_receipts from public, anon, authenticated;
revoke all on table analytics_offer_impression_receipts from public, anon, authenticated;
revoke all on table analytics_onboarding_journey_events from public, anon, authenticated;
...
comment on table analytics_viewer_daily is
  'Server-only self-scoped activity totals derived from canonical receipts; never an audience identity export.';
comment on table analytics_platform_commerce_daily is
  'Staff-only native-currency commerce totals from confirmed payment and posted ledger facts; never a balance.';
comment on table analytics_platform_operations_daily is
  'Staff-only bounded operational counts derived from canonical provider and worker state.';
comment on table analytics_retention_daily is
  'Staff-only privacy-suppressed active-user cohorts derived from canonical application sessions.';
comment on table analytics_onboarding_daily is
  'Staff-only privacy-suppressed onboarding funnel totals over the closed launch event vocabulary.';
comment on table analytics_profile_open_receipts is
  'Bounded idempotent profile-open facts. Actor identities remain server-only and are never returned by Analytics Core.';
comment on table analytics_offer_impression_receipts is
  'Bounded idempotent monetisation-offer impression facts; payment and entitlement domains remain authoritative.';
comment on table analytics_onboarding_journey_events is
  'Bounded typed onboarding facts only; no arbitrary event names, metadata, email, wallet address, or provider payload.';
