...
- Real push-provider send configuration, organization mutation workflows, and the complete `/admin/*` route-map breadth remain planned production gaps. They must be implemented as role-gated sanitized projections, not broad database or provider-payload exposure.
...
- notification projection, device health, and delivery queue health are visible through `GET /v1/admin/notifications/health`; real push-provider credentials remain a future provider slice
