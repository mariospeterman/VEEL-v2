...
| Event Access | Event/pass/check-in foundation and read-only admin Event Access ops projections exist. | Launch copy must avoid ticket-marketplace positioning; admin Event Access mutations, provider replay worker boundary, refund/transfer/resale remain out of launch unless separately approved. |
...
- Event Access admin reads may inspect event/pass/check-in state, but admin event/pass mutations and provider replay require dedicated audited worker-backed policy slices.
