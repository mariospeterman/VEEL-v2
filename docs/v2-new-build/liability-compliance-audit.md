...
| Notifications | OpenAPI routes, RLS-backed notification/preference/device tables, Fastify account routes, backend tests, settings preference reads, and admin health visibility exist. | Worker dispatch, service-worker subscription UX, actual push delivery, and Realtime subscription wiring remain. |
...
2. Complete notification delivery: worker/event dispatch, service-worker subscription UX, and Realtime account wiring.
