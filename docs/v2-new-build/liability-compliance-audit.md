...
| Refunds/disputes | User request routes, admin review routes, RLS-backed request table, sanitized projections, persisted idempotency/request-hash dedupe, payment-intent withdrawal-waiver evidence fields, durable receipt/in-app confirmation evidence, pending email-provider delivery rows, and audit events exist. | Launch-approved email delivery, creator-initiated noncustodial refund transaction evidence, policy-approved entitlement revocation/replacement execution, withdrawal-function UI for EU consumers where rights still exist, provider/counsel-reviewed copy, and jurisdiction-specific consumer disclosures remain. |
...
- Activity may open refund/access-issue review requests for non-delivery, technical failure, duplicate payment, fraud, misdescription, legal exceptions, and similar support cases. That surface must remain review-only and cannot promise automatic refunds, move funds, alter entitlement truth, create balances, or create payout queues.
...
5. Complete refund/dispute execution follow-up only after policy approval: launch-approved email confirmation delivery, EU withdrawal-function UI where rights still exist, creator-initiated noncustodial refund transaction evidence, entitlement revocation/replacement controls, consumer disclosures, platform-plan cancellation copy, and compliance ledger correction events.
