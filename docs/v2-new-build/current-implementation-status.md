...
- Media creation has backend draft, metadata/preview update, upload-session handoff, TUS browser upload/resume wiring, explicit publish submission, and a separate `publish_state`; it still needs deeper provider-ready status UI, entitlement-aware playback, Event Access draft linking, and abuse quota enforcement.
