...
3. Finish Supabase remote migration-history linking: session-pooler CLI access works, MCP is project-scoped, and migration `0056` is applied; the remaining blocker is reconciling remote timestamped migration names with local sequential filenames before future schema work depends on `supabase db push --dry-run`.
