...
- [x] Remove every inert production affordance or wire it to the canonical API owner. Mutuals choices now call the canonical interest mutation with retry-stable idempotency; unsupported language controls are absent; completed onboarding steps render as status rows rather than `#` links; offline retry explicitly reloads; and `pnpm frontend:affordances` rejects enabled type-button controls without an action owner plus empty/hash fallback links.
