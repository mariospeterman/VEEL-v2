...
- Payment settlement is native SOL devnet first; SPL/USDC, product-specific split settlement beyond the current content-unlock handoff, real transactional email domain/API-key configuration and staging deliverability smoke for withdrawal-waiver confirmations, exact subscription delegation program verification, and provider replay side-effect handlers still need launch-scope completion.
