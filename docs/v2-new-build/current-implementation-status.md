...
- Root Supabase CLI project is initialized with committed `supabase/config.toml`, repo-local Supabase CLI `2.106.0`, and `supabase/migrations` linked to the canonical `packages/database/migrations` SQL files. Supabase MCP is authenticated against the active `Veel v2` project and migration history is normalized to committed versions `0001` through `0062`. Root direct migration tooling uses the session-pooler wrapper successfully; `pnpm supabase:migrations` aligns local/remote history, `pnpm supabase:history:check` reports no missing/extra history rows, and Supabase advisors report no warning-level issues.
- Supabase/Auth session verification boundary, web SSR cookie refresh/confirmation route, landing-owned magic-link session UX, profile-completion mutation UI, external wallet challenge handoff UI, configured-session redirects, backend app-access redirects for protected app-shell pages, and backend session/profile readiness projections.
...
- External wallet challenge/link/revoke/status flow with backend signature verification and replay/expiry checks; landing onboarding and `/app/wallet` can now coordinate injected-wallet challenge signing while keeping wallet truth server-side; onramp provider boundary fails closed unless configured.
...
- Native SOL devnet payment intent, noncustodial creator split settlement facts, Solana Pay transaction-request URL, server-composed unsigned split transaction, submitted-signature capture, backend settlement verification, shared transaction boundary for payment submission settlement, content unlock entitlement grant after confirmed settlement, and browser support, content-unlock, live-pass, Event Access Pass, and paid-message handoffs to backend-created intents/transaction requests.
...
3. Extend real authenticated API/test-DB integration coverage beyond the wallet -> age -> create -> content unlock/refund request/payment activity receipt/Event Access/paid message/live pass/subscription/provider replay path to provider-specific webhook replay side-effect handlers and browser-authenticated refund/remediation admin evidence views. Supabase remote migration history is linked through `0062`; apply/link new migrations after each slice, rerun dry push/advisors, and keep public-table RLS at 100%.
