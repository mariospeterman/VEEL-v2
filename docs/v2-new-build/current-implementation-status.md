...
- Payment settlement is native SOL devnet first with server-composed creator split transactions for one-time creator monetization and a supported one-time USDC path. Recurring subscriptions are token-based only and fail closed without official Solana subscription/delegation verification; native SOL recurring subscriptions and real transactional email domain/API-key configuration plus staging deliverability smoke for withdrawal-waiver confirmations still need launch-scope completion. Provider replay now has canonical Bunny, Livepeer, and exact Helius/Solana side-effect recovery, recovery-only monotonic guards against stale media/replay regression, originating Solana provider-alias preservation, unsettled-first payment-intent matching with exact stored-signature confirmation reuse, tokenized retry/dead-letter behavior, redacted admin visibility, and audited requeue. Live provider event receipt/application remains transactionally retryable on redelivery and retains normalized replay facts for operator recovery.
