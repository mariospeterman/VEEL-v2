...
3. Extend real authenticated API/test-DB integration coverage beyond the wallet -> age -> create -> content unlock/refund request/Event Access/paid message/live pass/subscription/provider replay path to durable webhook replay, browser-authenticated money/access smoke flows, and user-visible receipt/withdrawal support surfaces. Supabase remote migration history is linked through `0056`; apply/link new migrations after this slice, rerun dry push/advisors, and keep public-table RLS at 100%.
