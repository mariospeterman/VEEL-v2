...
- Media creation has backend draft, daily draft/upload quota enforcement, metadata/preview update, persisted upload-session handoff, TUS browser upload/resume wiring, provider-status sync UI, explicit publish submission, and a separate `publish_state`; it still needs entitlement-aware playback, Event Access draft linking, and admin-tunable abuse policy surfaces.
