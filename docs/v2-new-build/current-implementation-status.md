...
- The current payment module manually constructs the transaction-request URL and route-specific web panels repeat wallet-handoff presentation. They remain real, supported code until Slice 06 replaces the encoder through the narrow Commerce Kit codec and converges the presentation into one shared checkout state machine; Launch 01 does not mix that refactor into identity/session work.
