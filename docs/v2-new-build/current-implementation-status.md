...
- Supabase/Auth session verification boundary, web SSR cookie refresh/confirmation route, real `/enter` magic-link session UX, profile-completion mutation UI, external wallet challenge handoff UI, configured-session redirects for protected web pages, and backend session/profile readiness projections.
...
3. Continue Supabase onboarding beyond `/enter`: backend app-access shell gating and age-provider handoff from the completed session/profile/external-wallet browser flow.
