...
- Home feed, content read model, media access projection, `/create` backend draft/upload-session handoff, Bunny/Livepeer provider status boundaries, and provider webhook normalization tests.
...
- Media creation has backend draft and upload-session handoff in the browser; it still needs full TUS byte upload/resumability, provider-ready plus moderation-pending separation, publish action, entitlement-aware playback, and abuse quota enforcement.
