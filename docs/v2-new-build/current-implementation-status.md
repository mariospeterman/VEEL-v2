...
- Privy is the only embedded-wallet launch runtime. Turnkey is not an active runtime or contract value. Native creator commerce is deferred; Vendure is the future canonical engine behind a WeVid-owned adapter, Medusa is the fallback, and Shopify may exist only as an optional external connector.
...
| DEFERRED | Recurring memberships/plans, Enterprise, expanded Mutuals, Vendure-backed native physical commerce, adult live, full AI/MCP, NFT/transferable passes, resale, and custom contracts | 08+, separately gated, or post-core |
...
- The production dependency graph no longer includes the unused `@solana/wallet-adapter-wallets` umbrella package, which removed unrelated hardware-wallet and WalletConnect adapters. Next.js is pinned to `16.3.0`, Playwright is pinned to the latest macOS 12-compatible release (`1.60.0`), and patched same-major `axios`/`ws` resolutions are enforced. Launch 00 `pnpm audit --prod` reports three high and two moderate transitive advisories: unpatched `bigint-buffer` through Solana SPL tooling, unpatched `image-size` through Privy/WalletConnect React Native tooling, and legacy `uuid` through Solana Web3/Jayson. No critical advisory is reported, but provider selection/dependency replacement and a clean production audit remain launch blockers; these findings are not dismissed merely because some paths do not appear in the built `.next` artifact.
...
- The wallet runtime is loaded on demand on landing and mounted once for the authenticated app. The configured embedded wallet button uses the official Privy SDK path and explicitly selects the Privy Solana wallet before signing; external wallets remain owned by Solana Wallet Adapter. Profile logout uses official Privy logout, Solana disconnect, Supabase local sign-out, app wallet-session cleanup, and server cookie expiry before replacing the location with `/`.
