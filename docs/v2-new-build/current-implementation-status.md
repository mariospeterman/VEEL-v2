...
- Media creation has backend draft, daily draft/upload quota enforcement, metadata/preview update, Event Access draft linking, persisted upload-session handoff, TUS browser upload/resume wiring, provider-status sync UI, entitlement-aware content playback rendering, explicit publish submission, and a separate `publish_state`; it still needs admin-tunable abuse policy surfaces.
