...
| Field | Current value |
| --- | --- |
| Latest merged baseline | `63f89cdad6955e4cd8024d2ab2652df72043c664` |
...
Repair PR #105 preserves the authenticated wallet session while an unknown identity explicitly
transitions into onboarding, keeps browser API routing canonical, and routes landing settlement copy
through the approved claim authority. Repair PR #106 pins the web runtime to Next.js `16.3.2`, clears
development-only PWA state, serializes `.next` preparation, and prevents stale mixed-runtime chunks
when localhost restarts. Both are part of the current protected-main baseline.
...
- Root Supabase CLI project is initialized with committed `supabase/config.toml`, repo-local Supabase CLI pinned to `2.113.0`, and `supabase/migrations` linked to the canonical `packages/database/migrations` SQL files. Local/CI startup runs through the repository wrapper, which constructs an ephemeral workdir containing forward migrations only; canonical `*.down.sql` rollback files are never presented to the Supabase migration runner. On 2026-08-26 the healthy `Veel v2` cloud project in `eu-west-1` was verified against the canonical source, the byte-identical timestamped `0090` history row was normalized to sequential version `0090`, and migrations `0091` through `0118` were applied through the linked repo-local CLI. Remote history now has all 118 sequential versions with no extras, a repeat dry-run reports the database up to date, Auth and REST health checks pass with the configured publishable key, and Supabase security/performance advisors report no warning- or error-level findings. Informational deny-all RLS and empty-database index notices remain visible for operator review. Generic application `DATABASE_URL` is never used by remote migration commands; API and worker runtime still require a separately provisioned pooled cloud `DATABASE_URL` before real-stack staging can pass.
...
- External wallet challenge/link/revoke/status flow with backend signature verification and replay/expiry checks; landing onboarding and `/app/wallet` coordinate Solana wallet-adapter challenge signing while keeping wallet truth server-side. Landing renders each detected Wallet Standard provider as a direct card, keeps the adapter modal as a fallback, and shows Privy as a peer only when its runtime env is configured. Active signing has an explicit stop/disconnect recovery and a bounded timeout instead of leaving every control disabled indefinitely; onramp remains fail closed unless configured.
...
- The external Solana wallet runtime loads when the landing login/onboarding frame opens and exposes installed Wallet Standard adapters directly through official `select` and `connect` behavior; the chooser is a fallback rather than a mandatory second layer. Privy remains unmounted until its configured peer card is chosen; that one click lazy-loads the official SDK and automatically opens its account flow. The runtime explicitly selects the Privy Solana wallet before signing, while external wallets remain owned by Solana Wallet Adapter. The authenticated app mounts both runtimes for wallet operations and provider logout. Profile logout uses official Privy logout, Solana disconnect, Supabase local sign-out, app wallet-session cleanup, and server cookie expiry before replacing the location with `/`.
- Landing login and onboarding use one accessible modal over the semantic public story while provider runtimes initialize or wallet UI takes focus. Known identities resume login; unknown identities explicitly transition to onboarding before account or embedded-wallet creation.
