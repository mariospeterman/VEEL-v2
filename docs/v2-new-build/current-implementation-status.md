...
- Protected-mutation idempotency is now classified by the lifetime of the underlying operation. Durable domain mutations keep entity- or receipt-backed keys without expiry; checkout consent joins content draft creation in retaining its generic actor/action/request-hash receipt at PostgreSQL `infinity`. Finite retention remains only for intrinsically bounded auth/wallet challenges, capabilities, payment/subscription windows, and seven-day feed-impression observations. Migration `0102` promotes existing `payment_checkout_consent` receipts; exact retries return the original result and changed-input reuse returns `409`.
