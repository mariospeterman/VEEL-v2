...
- Real authenticated API integration coverage now exercises the wallet -> age -> profile readiness -> content create -> content unlock -> confirmed settlement -> entitlement-backed unlock path against a configured Postgres/Supabase test database through Fastify routes and Postgres repositories. The test uses a real external-wallet Ed25519 signature, a signed Sumsub-style age webhook, backend-created profile/content/payment-intent mutations, a confirmed settlement verifier boundary, entitlement/read-projection assertions, deterministic cleanup, and an explicit remote-DB guard. Run it with `VEEL_ALLOW_REMOTE_API_INTEGRATION_TESTS=1 pnpm test:integration:api` when the root `.env` points at the intended non-production test database.
...
3. Extend real authenticated API/test-DB integration coverage beyond the wallet -> age -> create -> content unlock -> confirmed settlement path to Event Access, paid messages, live passes, subscriptions, and provider replay flows. Supabase remote migration history is now linked: Docker-backed shadow diff applied `0001` through `0056`, `migration repair` normalized the remote tracking rows, `pnpm supabase:push:dry` reports the remote database is up to date, remote advisors report no warn-level issues, and public-table RLS metadata reports 99/99 public tables with RLS enabled.
