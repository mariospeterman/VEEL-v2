...
- Payment settlement is native SOL devnet first; SPL/USDC, product-specific split settlement beyond the current content-unlock handoff, durable receipt/email confirmation for withdrawal-waiver records, exact subscription delegation program verification, and provider replay tooling still need launch-scope completion.
...
3. Extend real authenticated API/test-DB integration coverage beyond the wallet -> age -> create -> content unlock/refund request/Event Access/paid message/live pass/subscription/provider replay path to webhook replay and browser-authenticated money/access smoke flows. Supabase remote migration history is linked through `0056`; apply/link new migrations after this slice, rerun dry push/advisors, and keep public-table RLS at 100%.
