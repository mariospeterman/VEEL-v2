...
- Root Supabase CLI project is initialized with committed `supabase/config.toml`, repo-local Supabase CLI `2.106.0`, and `supabase/migrations` linked to the canonical `packages/database/migrations` SQL files. Supabase MCP is authenticated against the active `Veel v2` project and migration `0056_event_access_purchase_request_state` is applied remotely. Root direct migration tooling now uses the session-pooler wrapper successfully for migration listing; `pnpm supabase:migrations` works. `pnpm supabase:history:check` reports the remote mismatch without exposing SQL or secrets. `pnpm supabase:push:dry` still intentionally stops on remote timestamped migration history versus local sequential migration filenames until the history-linking decision is completed.
...
3. Finish Supabase remote migration-history linking: session-pooler CLI access works, MCP is project-scoped, migration `0056` is applied, and remote security/performance advisors currently report no warn-level issues. Do not run `migration repair` yet: the remote history table has 47 timestamped rows, local sequential history is missing for `0001` through `0056`, and only 4 name-matched remote rows are byte-for-byte aligned with committed SQL. The next repair attempt needs a backup plus a Docker-backed Supabase schema diff or equivalent MCP/management verification that proves the remote schema matches committed migrations before normalizing history.
