...
- Media creation has backend draft, daily draft/upload quota enforcement, metadata/preview update, upload-session handoff, TUS browser upload/resume wiring, explicit publish submission, and a separate `publish_state`; it still needs deeper provider-ready status UI, entitlement-aware playback, Event Access draft linking, and admin-tunable abuse policy surfaces.
