...
| `/passes` | Phase 3 | yes | yes | yes | events | My Passes and QR/access receipt list. |
| `/passes/:id` | Phase 3 | yes | yes | yes | events | Pass QR/details/check-in status. |
| `/event-access/:id` | Phase 3 | yes | yes | yes | events | Event detail/Event Access sheet. |
| `/app/tickets` | migration | yes | yes | yes | events | Compatibility alias until Passes routes replace it. |
| `/app/tickets/:id` | migration | yes | yes | yes | events | Compatibility alias until Pass details replace it. |
| `/app/events/:id` | migration | yes | yes | yes | events | Compatibility alias until Event Access route replaces it. |
...
| `/mutuals/mutuals` | Phase 4 | yes | yes | yes | mutuals | Mutual list and stale/archived state. |
| `/app/dating/*` | migration | yes | yes | yes | mutuals | Compatibility alias only until Mutuals migration completes. |
...
| Profiles | `GET /v1/profiles/:handle`, `GET /v1/profiles/me/creator-dashboard`, `PATCH /v1/profiles/me`, `POST /v1/follows/:userId` | MVP | Creator/user profile plus backend-derived creator monetisation dashboard. |
...
| Notifications | `GET /v1/notifications`, `PATCH /v1/notifications/:id/read`, `GET /v1/notifications/preferences`, `PATCH /v1/notifications/preferences`, `POST /v1/notifications/devices`, `DELETE /v1/notifications/devices/:id` | Phase 2 | Backend-derived notification projections and PWA push device registration. Notifications never create payment/access truth. |
...
| Organizations | `GET /v1/organizations` | Phase 2 | Studio/Enterprise member dashboard projection; software governance only, no custody, balances, withdrawals, or social preference. |
