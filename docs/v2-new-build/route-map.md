...
| --- | --- | --- | --- |
| Staff access | `GET /v1/admin/me`, `GET /v1/admin/staff`, `POST /v1/admin/staff/invitations`, `PATCH /v1/admin/staff/memberships/{membershipId}`, `GET /v1/staff/invitations/current`, `POST /v1/staff/invitations/{invitationId}/respond` | Release convergence | One canonical permission registry drives API policy and current-staff projection. Invitations target existing WeVid users, expire within seven days, require explicit acceptance, and role changes protect the final owner, revoke active sessions when access is suspended/revoked, notify affected staff, and write audited before/after state. |
...
| Engagement | `GET /v1/follows/:userId`, `POST /v1/follows/:userId`, `DELETE /v1/follows/:userId`, `POST /v1/engagement/:contentId/like`, `POST /v1/engagement/:contentId/save`, `POST /v1/engagement/:contentId/comments`, `GET /v1/engagement/:contentId/comments`, `POST /v1/engagement/comments/:commentId/like`, `POST /v1/shares` | MVP/Phase 2 | Backend-owned follow graph, aggregate projections, one-level comment conversations, safe mentions, comment likes, and consent-safe internal or external engagement records. Follow conveys no access, messaging, membership, or Mutuals authority. |
| Reports/Blocks | `POST /v1/reports`, `POST /v1/blocks/:userId`, `DELETE /v1/blocks/:userId` | MVP | Safety actions are idempotent and audited; a block disables either-direction follows and excludes social interaction. |
| Privacy | `GET /v1/privacy`, `POST /v1/mutes/:userId`, `DELETE /v1/mutes/:userId`, `POST /v1/privacy/data-requests` | Convergence 06 | Self-service block/mute state and auditable export/deletion workflow requests. Requests never claim immediate erasure and remain visible to the existing admin privacy queue. |
...
| Realtime | `POST /v1/realtime/token` | Phase 2 | Canonical application session mints a short-lived ES256 Supabase Realtime JWT plus its private `account:{userId}` topic; missing imported signing-key configuration fails closed. |
| Realtime | `POST /v1/realtime/telemetry` | Phase 2 | Records a bounded, typed connection-health event for account, conversation, or live private channels without storing topic identifiers or payload content. |
...
| Messages | `GET /v1/messages/conversations`, `POST /v1/messages/conversations`, `PATCH /v1/messages/conversations/:id/request`, `PATCH /v1/messages/conversations/:id/read`, `PATCH /v1/messages/conversations/:id/mute`, `GET /v1/messages/conversations/:id/messages`, `POST /v1/messages/conversations/:id/messages`, `PUT /v1/messages/conversations/:id/messages/:messageId/reactions/:reactionKey`, `DELETE /v1/messages/conversations/:id/messages/:messageId/reactions/:reactionKey`, `GET /v1/messages/conversations/:id/commercial-interactions`, `POST /v1/messages/conversations/:id/media-offers`, `PATCH /v1/messages/conversations/:id/media-offers/:offerId`, `POST /v1/messages/conversations/:id/media-offers/:offerId/payment-intents`, `POST /v1/messages/conversations/:id/creator-requests`, `PATCH /v1/messages/conversations/:id/creator-requests/:requestId`, `POST /v1/messages/conversations/:id/creator-requests/:requestId/payment-intents` | Phase 2 | Direct-message requests allow one introduction before recipient consent. Accepted conversations gate replies, revision-bound safe attachments, creator media offers, and the two-phase structured request/payment/delivery workspace. The retired paid-message creation route is removed; historical records remain readable and settle through the compatibility path. |
...
| MCP | `GET /.well-known/oauth-protected-resource`, `GET /.well-known/oauth-authorization-server`, `GET /oauth/authorize`, `POST /oauth/token`, `POST /oauth/revoke`, `GET /oauth/consent/:requestId`, `POST /oauth/consent/:requestId/approve`, `POST /oauth/consent/:requestId/deny`, `GET /v1/mcp/tools`, `GET /v1/mcp/connections`, `POST /v1/mcp/connections`, `GET /v1/mcp/connections/:id`, `POST /v1/mcp/connections/:id/revoke`, `POST /v1/mcp/media/uploads/{capabilityId}` | Phase 5 | External MCP connector management. Production private-data connectors use OAuth authorization-code plus PKCE with resource-bound bearer tokens. Scoped-token creation is dev/staging only. The media handoff redeems a hash-only one-time capability into the canonical private media authority and never publishes content. |
