...
| Analytics | `POST /v1/analytics/query`, `GET /v1/admin/analytics/health` | Convergence 03 | One authenticated structured query boundary over versioned typed projections. Scope, dimensions, native currency, privacy suppression, freshness, and current Enterprise consent are backend enforced; the admin route exposes projection health without audience identity or raw facts. |
