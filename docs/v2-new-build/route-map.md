...
| --- | --- | --- | --- |
| Auth/session | `POST /v1/auth/wallet/challenges`, `POST /v1/auth/wallet/sessions`, `GET /v1/session` | MVP | Wallet-first session creation plus frontend-safe app access, wallet, age, badges, prefs. Supabase email auth remains optional recovery/profile management. |
...
| Verification | `GET /v1/verification/status` | MVP | Backend-owned capability resolver across age access, creator KYC, and organization KYB. Frontend reads next action and missing requirements; it never computes verification truth. |
...
| Profiles | `GET /v1/profiles/:handle`, `GET /v1/profiles/me/creator-dashboard`, `GET /v1/profiles/me/creator-onboarding`, `PATCH /v1/profiles/me` | MVP | Creator/user profile plus backend-derived creator monetisation dashboard and Become Creator readiness checklist. Follow graphs are not in the active API contract until a real repository/schema slice exists. |
| Content | `GET /v1/content/feed`, `GET /v1/content/{contentId}`, `POST /v1/content`, `PATCH /v1/content/{contentId}`, `POST /v1/content/{contentId}/publish`, `POST /v1/media/uploads`, `POST /v1/media/assets/:id/sync` | MVP | Bunny-backed draft/upload/create/status; media upload sessions and publish are blocked unless backend capability resolver reports valid creator KYC. Draft creation can remain metadata-only after app access. |
...
| Payments | `POST /v1/payments/intents`, `GET /v1/payments/intents/:id`, `GET /v1/payments/intents/:id/transaction-request`, `POST /v1/payments/intents/:id/transaction-request`, `POST /v1/payments/intents/:id/submissions`, `POST /v1/content/:id/unlock-intents`, `POST /v1/webhooks/solana-indexer` | MVP | Noncustodial creator split payment intent, Solana Pay transaction request, content unlock intent, entitlement grant, and scoped Solana evidence. |
...
| Memberships | `GET /v1/subscriptions/plans`, `GET /v1/subscriptions`, `POST /v1/subscriptions/intents`, `POST /v1/subscriptions/authorizations/:id/submissions`, `PATCH /v1/subscriptions/:id/cancel` | Phase 2 | Compatibility route family for platform plans and Creator Memberships using official token-based Solana subscription authorization. Intent creation and renewal collection fail closed unless provider/program/RPC/mint/collector/merchant/on-chain verification are configured; native SOL recurring subscriptions are unsupported. Target copy says Memberships. |
