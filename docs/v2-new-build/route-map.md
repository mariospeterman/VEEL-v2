...
| Memberships | `GET /v1/platform-access`, `GET /v1/subscriptions/plans`, `GET /v1/subscriptions`, `POST /v1/subscriptions/intents`, `POST /v1/subscriptions/authorizations/:id/submissions`, `PATCH /v1/subscriptions/:id/cancel` | Phase 2 | Backend-owned five-tier platform catalog/current capability and allowance projection plus Creator Memberships using official token-based Solana subscription authorization. One active membership offer is allowed per profile at launch. Intent creation and renewal collection fail closed unless provider/program/RPC/mint/collector/merchant/on-chain verification are configured; native SOL recurring subscriptions are unsupported. |
