...
- Local macOS hosts install optional Darwin `rolldown` native bindings for Vitest/Vite and use `pnpm run doctor` to resolve the pinned Node.js/Corepack toolchain even when the interactive shell points at an older Node. Rolldown publishes its JavaScript runtime and platform bindings as one exact-version set, so the root arm64/x64 fallback pins must remain aligned with the resolved runtime and may only move in a coordinated runtime upgrade. Dependabot ignores standalone Darwin-binding updates, while `pnpm deps:check` fails on a manifest or lockfile mismatch. Do not bypass tests; local proof and the pinned Linux CI proof are both required before launch-scope changes are considered validated.
