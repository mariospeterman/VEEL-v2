...
| [`GHSA-w3rx-r6r6-pgpr`](https://github.com/advisories/GHSA-w3rx-r6r6-pgpr), `image-size@1.2.1`, high | Web → `@privy-io/react-auth@3.37.0` → WalletConnect/Reown → React Native Async Storage → React Native Metro CLI → `image-size`. This is transitive Metro tooling in the production dependency graph, not WeVid's server media parser. | The advisory requires a crafted ICNS buffer to be processed by `image-size`. WeVid does not import `image-size`, run Metro in the deployed Next.js service, or pass uploads to this chain. The local Next standalone trace contains no npm `image-size@1.2.1`, React Native, Metro, or affected `uuid` package; the separately bundled Next internal image utility is not this dependency. It is still tracked because the vulnerable package remains installed in the workspace graph. | No patched npm version exists. WeVid's media upload/probe path remains provider-backed and separate. Replace or upgrade the Privy connector graph when supported; the immutable staging OCI image must repeat the package/reachability proof before any explicit security acceptance. | Identity/wallet-provider dependencies; 2026-09-15 |
| [`GHSA-5p2g-fcmc-qvqq`](https://github.com/advisories/GHSA-5p2g-fcmc-qvqq), `image-size@1.2.1`, high | Same Privy → WalletConnect/Reown → React Native Metro CLI chain as the ICNS advisory. | The advisory requires a crafted JXL/HEIF buffer to be parsed by the transitive Metro utility. No WeVid request or upload route calls this package. | Same no-patch and replacement requirement as the ICNS advisory. Artifact reachability proof is a pre-production security gate, not permission to erase the finding. | Identity/wallet-provider dependencies; 2026-09-15 |
