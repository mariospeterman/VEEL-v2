...
- Home and Bits share one typed `FeedExperience` over the canonical server feed. Home presents mixed cards and Bits presents a vertical short-media viewport; both expose For you/Following tabs, stable cursor loading, route scroll restoration, active-item-only playback, next-poster preload, durable impressions, and canonical optimistic engagement/follow reconciliation.
...
- Content detail renders only backend-issued playback resources: Bunny embed resources use the official Player.js lifecycle API, Livepeer resources use the official React player, and blocked/teaser/not-ready states show access/readiness copy without local entitlement decisions. For qualifying free public VOD/live, the player starts a backend-owned usage session on play and sends ordered idempotent heartbeats only while playback is active and the page is visible; the backend caps credit and owns exhaustion. The unlock panel uses `POST /v1/content/{contentId}/unlock-intents` and the scoped Solana Pay checkout projection; the browser displays the backend-built wallet request only and entitlement remains backend-settlement-owned.
- `/app/subscriptions` presents the five platform tiers separately from profile-specific Creator Memberships. It reads `/v1/platform-access` for current tier and public-media usage, and does not expose provider mint/program/authorization internals as primary consumer UI.
