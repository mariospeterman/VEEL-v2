...
- Content detail now uses a browser mutation panel for `POST /v1/content/{contentId}/unlock-intents` and `GET /v1/payments/intents/{paymentIntentId}/transaction-request`. The browser displays the backend-built wallet request only; entitlement remains backend-settlement-owned.
- Create now calls `POST /v1/content`, `PATCH /v1/content/{contentId}`, `POST /v1/media/uploads`, and `POST /v1/content/{contentId}/publish` before using `tus-js-client` for Bunny uploads. Browser state is UX/cache only: metadata save, preview controls, upload completion, and submit-for-review do not approve moderation or create access.
