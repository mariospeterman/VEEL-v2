...
- Create now calls `POST /v1/content`, `PATCH /v1/content/{contentId}`, and `POST /v1/media/uploads` before using `tus-js-client` for Bunny uploads. Browser state is UX/cache only: metadata save, preview controls, and upload completion do not publish, approve moderation, or create access.
