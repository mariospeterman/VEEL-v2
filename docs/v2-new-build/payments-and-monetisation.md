...
- Every creator-recipient intent resolves its settlement wallet through `private.assert_recipient_monetisation_ready(...)` in the same database statement or transaction that creates the intent. The policy requires an active individual creator, enabled product, ready creator monetisation state, accepted tax state, current age and documentary creator-KYC evidence, and a configured user-owned recipient wallet on the requested chain. Browser recipient overrides, arbitrary linked-wallet fallback, and organization recipients without a separately approved organization policy fail closed.
