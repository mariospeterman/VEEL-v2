...
- Every one-time intent stores an immutable commercial quote: product-specific minimum, platform-fee bps, referral share of platform fee, quote time, expiry, policy source, and policy revision. Environment values provide safe defaults; an active row in `payment_commercial_policy_overrides` wins atomically during intent creation. Intent creation, checkout-consent acceptance, and lifetime-idempotent audited admin commercial-policy overrides use the shared Postgres transaction boundary, so policy/audit/replay receipts, quote/readiness/allocation writes, and consent/audit/idempotency writes remain all-or-nothing without introducing a second payment authority. The repository rejects a server- or buyer-supplied amount below that effective floor before it writes an intent.
- `GET /v1/admin/payments/commercial-policies` and replay-safe `PATCH /v1/admin/payments/commercial-policies/{productType}/{currency}` expose the dedicated staff-only override boundary. Each write is constrained, reasoned, revisioned, recorded in `audit_events`, and retained in the generic idempotency ledger for the logical operation lifetime. Payment policy never uses rollout feature flags.
- Quote expiry is also the maximum wallet-capability and settlement-evidence window. Transaction request creation, checkout lookup, submission, provider matching, and database settlement transitions fail closed after it; an old quote cannot be refreshed in place after pricing policy changes.
- Every new creator-recipient intent resolves its settlement wallet through `private.assert_recipient_monetisation_ready(...)` in the same database transaction that creates the intent. That gate consumes `private.resolve_recipient_monetisation_policy(...)`, whose deterministic precedence is required account override, global `required`/`disabled`, risk-based account exemption, configured product, normalized jurisdiction, then active normalized risk score. An untriggered `risk_based` policy does not silently become universal KYC; an exemption cannot weaken global `required`. The remaining requirements are an active individual creator, enabled product, accepted tax state, current age, and a configured user-owned recipient wallet on the requested chain. Browser recipient overrides and arbitrary linked-wallet fallback fail closed.
- Payment intents and creator Membership offers snapshot the resolved recipient-KYC requirement, mode, policy version, and reason with the commercial record. Recurring creator-Membership collection re-evaluates that resolver before leasing money movement; when KYC has become required and no valid purpose-specific evidence exists, the subscription is suspended, its next collection is cleared, and a redacted policy event is recorded before any transaction is submitted. Platform-plan collection does not invent a creator-recipient decision.
...
- `POST /v1/payments/intents/{paymentIntentId}/consent` atomically records the exact terms version and any required immediate-digital-access acknowledgement for the authenticated owner. The browser obtains checkout-terms acceptance and, when required, a separate express acknowledgement that immediate supply ends the applicable statutory withdrawal right once access begins. Its actor/action/request-hash replay receipt is retained for the lifetime of the logical consent operation because this is durable legal and business evidence: exact replays remain accepted and return the current canonical payment-intent state, while changed-input reuse conflicts even after the payment-intent window has elapsed. Capability creation, authenticated submission, provider-webhook matching, settlement transition, and the database transition guard all fail closed while required consent is absent.
- `GET /v1/payments/intents/{paymentIntentId}/transaction-request` is authenticated and, only after required consent, mints a short-lived, 32-byte checkout capability. Only its SHA-256 hash and a redacted URL are persisted; the raw capability is returned once in the wallet-facing `solana:https://...` URL together with a Commerce Kit-generated SVG QR data URL.
...
- `POST /v1/content/{contentId}/unlock-intents` creates or reuses a backend-priced `content_unlock` intent from active content access rules. Paid live events, Event Access Passes, accepted creator requests, creator media offers, platform plans, creator memberships, and content unlocks use product-specific backend-priced endpoints instead of the generic payment-intent endpoint.
...
- Creator media-offer payment uses the existing `content_unlock` authority and grants only the defined content entitlement. Structured creator-request payment becomes available only after the named creator accepts exact terms; verified settlement then rechecks consent before activating the delivery workspace. Historical `paid_message` records remain a settlement-compatibility detail, with no creation endpoint or new-product UI.
...
- Real API/test-DB coverage verifies the `event_access_pass` route sequence through concurrent backend intent creation against capacity-one inventory, exactly one active short reservation, cancellation of the rejected orphan intent, consent, confirmed settlement submission, wallet transaction and settlement attempt rows, access purchase request state, active Event Access Pass row, audit event, and access-pass activity projection. Reservation, free grant, and paid settlement inventory locks occur in separate transaction statements so waiters receive fresh post-lock PostgreSQL snapshots. Invalid signatures never extend reservations. If a valid onchain payment can no longer be delivered because its inventory was reallocated, payment truth remains confirmed while the backend creates one audited `pending_internal` access support case; it never silently records an access grant.
- Convergence 05 coverage verifies that the retired paid-message endpoint is absent, creator-request payment is rejected before creator acceptance, media offers bind to approved content, and verified settlement activates only the consent-safe entitlement or delivery workspace. Historical paid-message settlement coverage remains to prevent orphaning old records.
...
- Real API/test-DB coverage verifies the delegated subscription boundary through backend plan listing, authorization intent creation, verifier-scoped evidence submission, pending subscription projection, authorization intent/event rows, worker collection guards, and provider-event replay state updates. Provider-event recovery uses the canonical Bunny, Livepeer, and Helius/Solana application authorities: Solana replay includes confirmed intents only for idempotent recovery, skips a second settlement write when payment truth is already confirmed, and otherwise repeats exact backend verification before the existing transactional submission path. Active recurring access remains fail-closed until official Solana subscription provider verification is configured.
- Real API/test-DB coverage verifies refund/dispute request behavior after confirmed content unlock settlement: the request uses the shared Postgres transaction boundary, is idempotent by persisted request hash, writes one audit event, keeps payment and entitlement truth unchanged, and exposes only the no-custody review boundary.
...
- `/subscriptions` requests `POST /v1/subscriptions/intents`, receives a server-derived unsigned recurring-delegation transaction from `GET /v1/subscriptions/authorizations/{authorizationIntentId}/transaction`, asks the connected wallet to approve it once, and submits only the signature to `POST /v1/subscriptions/authorizations/{authorizationIntentId}/submissions`. No authority, delegation, token-account, limit, period, nonce, or expiry field is typed or trusted in the browser. Verified authorization moves the subscription to `renewal_pending`; only the worker's confirmed first collection activates access. Cancellation uses `PATCH /v1/subscriptions/{subscriptionId}/cancel` and preserves already-paid access through the recorded period end.
...
- The worker owns the renewal tick: it leases only active/renewal-pending/grace, launch-approved, token-based recurring delegations from `subscriptions_next_collection_idx`, records one durable `subscription_collections` row and provider idempotency key per period, and submits one atomic transaction containing the official `transferRecurring` instructions for the exact nonzero creator/platform recipients. Its operational collector signer is server-only, is checked against the configured public key at startup, pays network/ATA rent only, and cannot exceed the user's per-period delegation. The first confirmed collection activates access; later confirmations advance the period. Lease ownership is token-guarded, stale leases are reclaimable, atomic amounts remain `bigint`, and every retry after the first scans finalized delegation transactions for the collection memo before another debit. Unknown reconciliation fails closed. Cancelled, revoked, expired, unconfigured, native-SOL, split-mismatched, or provider-mismatched subscriptions are not collected.
- `pnpm proof:subscriptions` is the exact staging gate. It fails with `CODE_COMPLETE_PROVIDER_BLOCKED` while credentials/evidence are absent, validates the canonical executable program, supported token mint, collector key/public-address match, finalized setup and collection transactions, confirmed backend collection/access state, exact split sum, and plan launch approval. No secret or full signature is printed.
...
- Provider-event replay uses the same lease, backoff, attempt-ceiling, queue-health, and audited recovery policy. Admin provider-event rows expose only the replay request ID, state, timing, and redacted failure code needed to recover a dead-letter job; raw provider payloads remain server-only.
...
- approved creator media offer or creator-accepted structured request (`paid_message` remains an internal compatibility discriminator only)
- Profile Membership
...
One-time WeVid products remain server-priced SOL/approved USDC intents with backend chain verification. An onramp only funds the user-owned wallet and never grants access. Slice 06 exact-pins `@solana-commerce/solana-pay` `0.1.1` at reviewed upstream commit `6164d5104f3d1bd4cfbb637075f000d6ac23d6c3` behind a narrow codec for standards-compliant URL query encoding and SVG QR/deep-link handoff. Its callback, parser, QR, wallet state, or generic verifier can never confirm settlement or grant a domain outcome.

The reviewed `0.1.1` encoder/parser pair is not internally round-trip compatible for transaction-request links: its encoder returns the HTTPS callback directly while its parser accepts only `solana:` and removes two pathname characters. The WeVid codec therefore uses the official encoder only to produce the package-owned query shape, wraps the validated callback in the standard `solana:` scheme, validates the result locally, and uses the official QR primitive. Compatibility tests pin this behavior and reject transfer-request, non-HTTPS, and unsafe callback shapes. Replacing this workaround requires a reviewed upstream version plus the same round-trip, bundle, and wallet proof.
...
One reusable WeVid checkout presentation serves Support, content unlock, creator media offers, accepted structured creator requests, Event Access, paid live, and later physical products. Its implemented browser state is `idle -> creating -> review -> wallet -> submitted -> confirmed | error`; the backend owns payment and domain truth after intent creation. Review shows the exact total/asset, product, creator-side proceeds, platform fee, checkout-terms acceptance, and a separate immediate-access/withdrawal-right acknowledgement when required before any wallet capability is created. A connected Wallet Standard adapter can approve directly; otherwise the same opaque backend capability is available through an accessible SVG QR and deep link. QR, deep-link, and connected-wallet paths all poll the backend projection, remain non-final until backend confirmation, and reuse one logical idempotency key across a lost-response retry.

Shared presentation does not collapse outcomes: Support creates a receipt only; content and media offers create exact entitlements; an accepted structured creator request activates its defined delivery workspace; Event Access creates a Pass; paid live creates live/replay access; a future physical product creates a paid Order/Fulfillment state. The blockchain remains payment truth and each owning domain remains outcome truth.
...
- Creator content, media offers, accepted structured deliverables, paid live events, Event Access Passes, creator Memberships, and creator support are creator-sold products where the creator/event owner is the seller/responsible party where legally supportable.
...
POST /v1/payments/intents/:id/consent
...
- create intent
- idempotency reuse
- exact checkout consent and changed-replay conflict
- transaction/submission rejection before required consent
- transaction request
- Commerce Kit URL/QR compatibility and unsafe-link rejection
- wallet submission
- native SOL confirm
- SPL confirm
- wrong payer
...
- referral commission
- self-referral blocked
- canonical earnings wallet ownership, exact replay, changed replay, and audit
- capacity-one concurrent Event Access reservation and orphan-intent cancellation
