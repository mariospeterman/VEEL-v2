...
- Confirmed payment settlement records durable confirmation evidence: one receipt, receipt line, compliance-ledger payment-settled entry, in-app confirmation delivery, pending email-provider delivery state, notification projection, and audit event. Email confirmation remains `provider_not_configured` until a launch-approved transactional email provider is wired; the browser must not fake durable confirmation delivery.
