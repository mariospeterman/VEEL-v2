...
- Every one-time intent stores an immutable commercial quote: product-specific minimum, platform-fee bps, referral share of platform fee, quote time, expiry, policy source, and policy revision. Environment values provide safe defaults; an active row in `payment_commercial_policy_overrides` wins atomically during intent creation. The repository rejects a server- or buyer-supplied amount below that effective floor before it writes an intent.
