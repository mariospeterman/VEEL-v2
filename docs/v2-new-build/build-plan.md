...
| 08 | Recurring platform subscriptions and creator memberships | `CODE_COMPLETE_PROVIDER_BLOCKED` on `codex/launch-08-recurring-memberships`; local contract, migration, API, worker, web, operations, build, unit, provider-boundary, and real-Postgres proof are complete. Real Solana devnet collector and authorization/collection evidence remain the pre-production gate. |
