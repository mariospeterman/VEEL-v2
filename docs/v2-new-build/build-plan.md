...
| 02 | Three-step onboarding, canonical session, optional recovery, and earnings readiness | `CODE_COMPLETE_PROVIDER_BLOCKED` and merged through PR #72 at `4860133`: wallet-first entry, canonical opaque sessions, optional recovery, three-step onboarding, and backend-owned Enable Earnings access use canonical `users.id`; migration `0107` prevents the transitional compatibility column from becoming a second identity authority. Real recovery, wallet, age, and KYC provider acceptance remains a pre-production gate. |
...
| 07 | SFW live host, viewer, chat, and operational controls | `CODE_COMPLETE_PROVIDER_BLOCKED` and merged through PR #48 at `2cc1712`; protected CI, isolated Postgres, migration rollback/reapply, worker/route tests, and 36 desktop/mobile browser journeys passed. Real Livepeer staging credentials and signed provider evidence remain pre-production gates. |
| 08 | Recurring platform subscriptions and creator memberships | `CODE_COMPLETE_PROVIDER_BLOCKED` and merged through PR #49 at `c7717a5`; protected CI, isolated Postgres migration/rollback, real-Postgres integration, provider-boundary, build, and desktop/mobile browser proof passed. Real Solana devnet collector and authorization/collection evidence remain the pre-production gate. |
...
| Convergence 09 | Release convergence pass | Ordered operator harness and RBAC, final product UX, and cloud/provider acceptance. The pass stays fail closed and may end only at `OPERATOR_READY_FOR_STAGING_CONFIGURATION` or `STAGING_PROVEN`; it never enables production launch. |
...
| Convergence 09B — Product and UI convergence | Release convergence B | Active ordered slice: Studio/Enterprise separation, Moments and scheduled publication, and locked-mockup responsive product polish over existing authorities. |
| Convergence 09C — Cloud and provider acceptance | Release convergence C | Runs only after 09B merges; consumes owner-managed staging/provider configuration and may end only at `OPERATOR_READY_FOR_STAGING_CONFIGURATION` or evidence-backed `STAGING_PROVEN`. |
