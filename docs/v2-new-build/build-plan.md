...
| 11 | Actual deployment, observability, recovery, and legal launch | Active on PR #52: runtime-neutral non-root OCI targets, post-green-main attested image digests, immutable release manifest, exact-artifact staging/production workflow gates, OTLP API/worker bootstrap, synthetic/bounded-load tooling, backup/restore proof tooling, incident/rollback/legal runbooks, and provider-blocked status reporting are implemented. Hosting/OIDC, shared credentials, provider dashboard proof, alert destinations, final legal approval, and explicit production approval remain external pre-production gates. |
