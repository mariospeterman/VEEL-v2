...
| 02 | Three-step onboarding, canonical session, optional recovery, and earnings readiness | Launch 02B is active on `codex/launch-02-canonical-access`: the existing backend-owned Enable Earnings journey is being moved with entry, age, wallet, and profile access onto canonical `users.id` lookups that work with the transitional legacy identity column null. Real provider acceptance evidence remains a launch gate. |
...
| 11 | Actual deployment, observability, recovery, and legal launch | Release operations merged through PRs #52–#54 at `fba30a2`: runtime-neutral non-root OCI targets, post-green-main attested image digests, immutable release manifest, exact-artifact promotion boundaries, OTLP bootstrap, synthetic/bounded-load tooling, backup/restore proof tooling, and incident/rollback/legal runbooks are implemented. Launch 11B on PR #55 makes absent staging configuration/evidence and the missing hosting adapter an honest non-zero gate and completes the redacted proof matrix. Hosting/OIDC, shared credentials, provider dashboards, alert destinations, final legal approval, and explicit production approval remain external pre-production gates. |
