...
export function MediaAssetComposer({
  ...
  return (
    <section className="overflow-hidden rounded border border-(--line) bg-(--panel)">
      ...
      {allUploaded ? <section aria-live="polite" className="grid gap-3 border-t border-(--line) p-4 sm:p-5"><p className="text-sm leading-6 text-(--muted)">Upload complete. Publication stays blocked until every provider and safety review returns complete release evidence.</p>{adultSelected && !adultPublishReady ? <AdultPublisherGate verification={verification} /> : null}<button className="justify-self-start rounded bg-(--foreground) px-4 py-2 text-sm font-semibold text-(--background) disabled:opacity-50" disabled={pending !== null || submitted || !accepted || (adultSelected && !adultPublishReady)} onClick={() => void onPublish()} type="button">{pending === "publish" ? "Checking readiness…" : submitted ? "Submitted for review" : "Review and submit"}</button></section> : null}
